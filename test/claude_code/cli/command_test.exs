defmodule ClaudeCode.CLI.CommandTest do
  ...
  describe "to_cli_args/1" do
    ...
    test "marketplace plugin string (with @) merges into settings.enabledPlugins" do
      opts = [plugins: ["code-simplifier@claude-plugins-official"]]
      ...
      {:ok, settings} = Jason.decode(settings_json)
      assert settings["enabledPlugins"]["code-simplifier@claude-plugins-official"] == true
    end

    test "marketplace plugin map merges into settings.enabledPlugins" do
      opts = [
        plugins: [%{type: :marketplace, id: "code-simplifier@claude-plugins-official"}]
      ]
    ...
    test "mixed local and marketplace plugins are partitioned correctly" do
      opts = [
        plugins: [
          "./my-local-plugin",
          "code-simplifier@claude-plugins-official",
          %{type: :local, path: "./another-local"},
          %{type: :marketplace, id: "pr-review@my-org"}
        ]
      ]

      args = Command.to_cli_args(opts)

      # Local plugins become --plugin-dir flags
      plugin_dir_count = Enum.count(args, &(&1 == "--plugin-dir"))
      assert plugin_dir_count == 2
      assert "./my-local-plugin" in args
      assert "./another-local" in args

      # Marketplace plugins go into --settings
      assert "--settings" in args
      settings_idx = Enum.find_index(args, &(&1 == "--settings"))
      settings_json = Enum.at(args, settings_idx + 1)
      {:ok, settings} = Jason.decode(settings_json)
      assert settings["enabledPlugins"]["code-simplifier@claude-plugins-official"] == true
      assert settings["enabledPlugins"]["pr-review@my-org"] == true
    end

    test "marketplace plugins merge with existing settings map" do
      opts = [
        settings: %{"customSetting" => "value", "enabledPlugins" => %{"existing@org" => true}},
        plugins: ["new-plugin@org"]
      ]
      ...
      # Existing enabledPlugins preserved
      assert settings["enabledPlugins"]["existing@org"] == true
      # New plugin added
      assert settings["enabledPlugins"]["new-plugin@org"] == true
    end

    test "marketplace plugins merge with existing settings JSON string" do
      existing_json = Jason.encode!(%{"enabledPlugins" => %{"existing@org" => true}})
      opts = [settings: existing_json, plugins: ["new-plugin@org"]]
    ...
    test "only local plugins produces no settings change" do
      opts = [plugins: ["./local-only"]]

      args = Command.to_cli_args(opts)
      assert "--plugin-dir" in args
      # Settings should only have the setting-sources empty string, not enabledPlugins
      settings_idx = Enum.find_index(args, &(&1 == "--settings"))
