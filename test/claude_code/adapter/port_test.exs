defmodule ClaudeCode.Adapter.PortTest do
  ...
  describe "filter_system_env/0" do
    test "includes vars with CLI-recognized prefixes" do
      System.put_env("ANTHROPIC_TEST_KEY", "val1")
      System.put_env("CLAUDE_CODE_TEST_FLAG", "val2")
      System.put_env("CLAUDE_CONFIG_DIR_TEST", "val3")
      System.put_env("VERTEX_REGION_TEST", "val4")

      try do
        env = Port.filter_system_env()
        assert env["ANTHROPIC_TEST_KEY"] == "val1"
        assert env["CLAUDE_CODE_TEST_FLAG"] == "val2"
        assert env["CLAUDE_CONFIG_DIR_TEST"] == "val3"
        assert env["VERTEX_REGION_TEST"] == "val4"
      after
        System.delete_env("ANTHROPIC_TEST_KEY")
        System.delete_env("CLAUDE_CODE_TEST_FLAG")
        System.delete_env("CLAUDE_CONFIG_DIR_TEST")
        System.delete_env("VERTEX_REGION_TEST")
      end
    end

    test "includes non-namespaced CLI vars from allowlist" do
      System.put_env("HTTP_PROXY", "http://proxy:8080")
      System.put_env("NO_PROXY", "localhost")
      System.put_env("MAX_THINKING_TOKENS", "4096")
      ...
      after
        ...
        System.delete_env("MAX_THINKING_TOKENS")
      end
    end

    test "includes essential system vars" do
      env = Port.filter_system_env()
      assert Map.has_key?(env, "PATH")
      assert Map.has_key?(env, "HOME")
    end

    test "includes LC_ locale vars" do
    ...
    test "excludes unrecognized vars" do
      System.put_env("SSH_PRIVATE_KEY_DONT_LEAK", "secret")
      System.put_env("DATABASE_URL", "postgres://secret")
      System.put_env("AWS_SECRET_ACCESS_KEY", "supersecret")

      try do
        env = Port.filter_system_env()
        refute Map.has_key?(env, "SSH_PRIVATE_KEY_DONT_LEAK")
        refute Map.has_key?(env, "DATABASE_URL")
        refute Map.has_key?(env, "AWS_SECRET_ACCESS_KEY")
      after
        System.delete_env("SSH_PRIVATE_KEY_DONT_LEAK")
        System.delete_env("DATABASE_URL")
        System.delete_env("AWS_SECRET_ACCESS_KEY")
      end
    end

    test "user env bypasses filter (passed through unfiltered)" do
      # User-provided :env should always be included even if not in allowlist
      env = Port.build_env([env: %{"MY_CUSTOM_SECRET" => "value123"}], nil)
      assert env["MY_CUSTOM_SECRET"] == "value123"
    end
  end
