defmodule ClaudeCode.Message.SystemMessage.TaskStartedTest do
  ...
  describe "new/1" do
    test "parses a valid task_started message with all fields" do
      ...
      assert {:ok, message} = TaskStarted.new(json)
    ...
    test "returns error for missing task_id" do
      ...
      assert {:error, :missing_required_fields} = TaskStarted.new(json)
    end
    ...
    test "returns error for invalid message type" do
      json = %{"type" => "assistant"}
      assert {:error, :invalid_message_type} = TaskStarted.new(json)
    end
  end

  describe "task_started?/1" do
    test "returns true for a TaskStarted struct" do
      message = %TaskStarted{
        type: :system,
        subtype: :task_started,
        task_id: "task-1",
      ...
      assert TaskStarted.task_started?(message) == true
    end

    test "returns false for other values" do
      assert TaskStarted.task_started?(%{}) == false
      assert TaskStarted.task_started?(nil) == false
      assert TaskStarted.task_started?("string") == false
    end
  end
end
