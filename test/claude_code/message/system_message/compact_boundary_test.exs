defmodule ClaudeCode.Message.SystemMessage.CompactBoundaryTest do
  ...
  describe "new/1" do
    test "parses a valid compact boundary message with manual trigger" do
      ...
      assert {:ok, message} = CompactBoundary.new(json)
    ...
    test "returns error for invalid message type" do
      ...
      assert {:error, :invalid_message_type} = CompactBoundary.new(json)
    end
    ...
    test "returns error for missing uuid" do
      ...
      assert {:error, {:missing_fields, missing}} = CompactBoundary.new(json)
  ...
  describe "type guards" do
    test "compact_boundary?/1 returns true for compact boundary messages" do
      {:ok, message} = CompactBoundary.new(valid_compact_boundary_json())
      assert CompactBoundary.compact_boundary?(message)
    end

    test "compact_boundary?/1 returns false for non-compact-boundary messages" do
      refute CompactBoundary.compact_boundary?(%{type: :system, subtype: :init})
      refute CompactBoundary.compact_boundary?(%{type: :assistant})
      refute CompactBoundary.compact_boundary?(nil)
      refute CompactBoundary.compact_boundary?("not a message")
      refute CompactBoundary.compact_boundary?(%{})
    end
  end
  ...
  defp compact_boundary_fixture(attrs \\ %{}) do
    ...
    struct!(CompactBoundary, Map.merge(defaults, attrs))
  end
end
