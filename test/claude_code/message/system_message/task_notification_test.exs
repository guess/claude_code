defmodule ClaudeCode.Message.SystemMessage.TaskNotificationTest do
  ...
  describe "new/1" do
    test "parses a valid task_notification message with all fields" do
      ...
      assert {:ok, message} = TaskNotification.new(json)
    ...
    test "returns error for missing required fields" do
      ...
      assert {:error, :missing_required_fields} = TaskNotification.new(json)
    end

    test "returns error for invalid message type" do
      json = %{"type" => "assistant"}
      assert {:error, :invalid_message_type} = TaskNotification.new(json)
    end
  end

  describe "task_notification?/1" do
    test "returns true for a TaskNotification struct" do
      ...
      assert TaskNotification.task_notification?(message) == true
    end

    test "returns false for other values" do
      assert TaskNotification.task_notification?(%{}) == false
      assert TaskNotification.task_notification?(nil) == false
      assert TaskNotification.task_notification?("string") == false
    end
  end
