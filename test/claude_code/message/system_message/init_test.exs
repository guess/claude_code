defmodule ClaudeCode.Message.SystemMessage.InitTest do
  ...
  describe "new/1" do
    test "parses a valid system init message" do
      ...
      assert {:ok, message} = Init.new(json)
    ...
    test "returns error for invalid type" do
      json = %{"type" => "assistant"}
      assert {:error, :invalid_message_type} = Init.new(json)
    end

    test "returns error for missing required fields" do
      json = %{"type" => "system", "subtype" => "init"}
      assert {:error, {:missing_fields, _}} = Init.new(json)
    end
  end

  describe "type guards" do
    test "init?/1 returns true for init messages" do
      {:ok, message} = Init.new(valid_system_json())
      assert Init.init?(message)
    end

    test "init?/1 returns false for non-init messages" do
      refute Init.init?(%{type: :assistant})
      refute Init.init?(nil)
      refute Init.init?("not a message")
    end
  end
