...
echo "Say hello" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/simple_hello.json || true

echo "📝 Capturing math calculation..."
echo "What is 2 + 2?" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/math_calculation.json || true

echo "📝 Capturing file listing (potential tool use)..."
echo "What files are in the current directory?" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/file_listing.json || true

echo "📝 Capturing file creation (tool use)..."
echo "Create a file named test.txt with the content 'Hello from Claude'" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/create_file.json || true

echo "📝 Capturing file read (tool use)..."
echo "Read the README.md file and tell me what this project is about" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/read_file.json || true

echo "📝 Capturing error case..."
echo "Read a file that does not exist: /nonexistent/file.txt" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/error_case.json || true
...
echo "📝 Capturing complex tool chain..."
echo "Read the mix.exs file, extract the version number, and create a VERSION.txt file with that version" | claude --output-format stream-json --verbose --print 2>&1 > test/fixtures/cli_messages/complex_tool_chain.json || true
