...
- Use `ClaudeCode.query/2` for one-off examples that don't require streaming or multi-turn sessions. Reserve `ClaudeCode.start_link/1` + `ClaudeCode.stream/3` for examples that inspect intermediate messages or demonstrate streaming output.
